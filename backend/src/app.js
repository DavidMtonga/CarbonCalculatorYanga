...
app.post('/api/calculations', authenticateToken, async (req, res) => {
  try {
    const { type, data, emissions, offset } = req.body;
    
    // Validate input
    if (!type || !data || emissions === undefined || offset === undefined) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
...
app.get('/api/admin/stats', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      totalOffset: offsetAgg._sum.offset || 0
    });
  } catch (error) {
...
app.get('/api/admin/export', authenticateToken, async (req, res) => {
  try {
    ...
    // Convert to CSV
    const csvString = [
      ['ID', 'User Name', 'User Email', 'Organization', 'Calculation Type', 'Emissions (kg CO₂e)', 'Offset (kg CO₂e)', 'Date'],
      ...csvData.map(item => [
        item.id,
        `"${item.userName}"`,
        item.userEmail,
